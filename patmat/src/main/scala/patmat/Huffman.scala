...
object Huffman {
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def helper(tree: CodeTree)(char: Char): List[Bit] = tree match {
        case Leaf(_, _) => List()
        case Fork(left, right, _, _) => if (chars(left).contains(char)) 0 :: helper(left)(char) else 1 :: helper(right)(char)
      }
   ...
   * on the two parameter code tables.
    * Tree a goes to the left side, tree b to the right side. Assign 0 and 1 before.
   */
